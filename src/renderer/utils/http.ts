 ...
 */
const tipMsg = debounce(
  (
    msg: string,
    type: "success" | "info" | "warning" | "error" | "loading" = "error"
  ) => {
  ...
  return (config) => {
    ...
    const servers: any = async () => {
      ...
      try {
        ...
        console.log(
          `%c【ERROR】请求: ${config.baseURL + config.url} 失败, ${
            retryDelay / 1000
          } 秒后重试...`,
          "color:red"
        );
    ...
    if (method === "get" && useCache) {
      ...
      if (!responsePromise || forceUpdate) {
        ...
        console.log(
          `%c【TIP】: ${config.baseURL + config.url} 数据, 命中缓存。`,
          "color:#4caf50"
        );
        return responsePromise;
      }
  ...
  try {
    ...
    if (beforRefreshToken) {
      ...
      const result = await instance.post(
        "/cloud/machineAccount/refresh-token",
        {
          refreshToken: `Bearer ${beforRefreshToken}`,
        }
      );
...
// 业务错误处理
async function handleAuthError(config: AxiosRequestConfig, data: any) {
...
http.interceptors.request.use(
  (config) => {
    const { loading, loadingText } = config as InternalAxiosRequestConfig &
...
http.interceptors.response.use(
  (response) => {
    const { loading, successMessage } =
      response.config as InternalAxiosRequestConfig & CustomAxiosRequestConfig;
